...
class World():
    ...
    def __init__(self,string):
        ...
        self.rcl_l = [[None for j in range(12)]
                      for i in range(10000 + 1)]#ここを直す

        self.rcl_r = [[None for j in range(12)]
                      for i in range(10000 + 1)]#ここを直す

        self.playmode = []
        file_name = argvs.split(".")[0]
    ...
    def ourPlayer_x(self,unum,cycle):
        teammate = []
        teammate.append(PlayerObject(0.0, 0.0, 0))
        pattern = " \(\(l [0-9]+\) "
        match = self.rcg[cycle]
        ...
        return teammate[unum].pos().x
    
    def ourPlayer_y(self,unum,cycle):
        teammate = []
        teammate.append(PlayerObject(0.0, 0.0, 0))
        pattern = " \(\(l [0-9]+\) "
        match = self.rcg[cycle]
        ...
        return teammate[unum].pos().y

    def ourPlayer_pos(self,unum,cycle):
        teammate = []
        teammate.append(PlayerObject(0.0, 0.0, 0))
        pattern = " \(\(l [0-9]+\) "
        match = self.rcg[cycle]
        ...
        return teammate[unum].pos()
    
    def theirPlayer_x(self,unum,cycle):
        opponent = []
        opponent.append(PlayerObject(0.0, 0.0, 0))
        pattern = " \(\(r [0-9]+\) "
        match = self.rcg[cycle]
        ...
        return opponent[unum].pos().x

        
    def theirPlayer_y(self,unum,cycle = 0):
        opponent = []
        opponent.append(PlayerObject(0.0, 0.0, 0))
        pattern = " \(\(r [0-9]+\) "
        match = self.rcg[cycle]
